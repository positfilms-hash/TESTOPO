  ...
  implements QuestionReviewFeedbackRepository
{
  ...
  async create(
    ...
    const row = await this.port.table('question_review_feedback').insert({
      ...
      opposition_id: feedback.opposition_id,
      feedback_type: feedback.feedback_type,
      severity: feedback.severity,
      comment: feedback.comment,
      generation_run_id: feedback.generation_run_id,
      suggested_fix: feedback.suggested_fix,
      source_issue: feedback.source_issue,
...
function toFeedback(row: SupabaseRow): QuestionReviewFeedback {
  return {
    ...
    generation_run_id: asNullableString(row.generation_run_id),
    suggested_fix: asNullableString(row.suggested_fix),
    source_issue: asNullableString(row.source_issue),
