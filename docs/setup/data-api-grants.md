...
## Si una tabla "no existe" al aplicarla

`ERROR: relation "public.<tabla>" does not exist` significa que el proyecto **no
tiene aún todo el esquema** (faltan migraciones previas, p. ej. 022–032). La
migración 033 es **tolerante**: recorre la lista y concede solo sobre las tablas
que existen (avisa con `NOTICE: omitida (no existe aun)`), así que **no falla**.
Pero la app necesita esas tablas para funcionar: aplica primero **todo el historial
de migraciones en orden** (`0001` → `032`) y luego (o de nuevo) la `033`, que es
idempotente y completará los grants restantes.

## Aplicar la migración

Sigue el runbook general de migraciones
([`migrations-runbook.md`](./migrations-runbook.md)). Lo más fiable es aplicar
**todas** las migraciones del repo en orden al proyecto vinculado (p. ej.
`supabase db push`), no solo la 033 suelta. Es una migración SQL ordinaria; no
requiere despliegue de código ni cambios de secretos.
