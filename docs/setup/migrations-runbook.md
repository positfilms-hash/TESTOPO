...
| 029 | `029_secure_test_question_access.sql` | Cierre del gap `is_correct` (BUG-001): `questions`/`question_options` pasan a SELECT solo para gestores; vistas seguras `safe_questions`/`safe_question_options` (sin solución) para el alumno; RPC `SECURITY DEFINER` `submit_attempt`/`get_attempt_review` para corregir/revisar. Requiere desplegar el código de la app a la vez. Ver [rls-known-gaps.md](../security/rls-known-gaps.md) |
| 030 | `030_syllabus_index.sql` | Índice de temario IA (SPEC 019 + 028-D) a Supabase: 7 tablas (`syllabus_index_runs`/`_proposals`/`_node_proposals`, `material_topic_suggestions`, `exam_pattern_summaries`, `syllabus_index_node_sources`, `topic_source_references`) con RLS de **solo gestión** (alumno sin acceso). Cierra el último dominio InMemory del MVP. Ver [ai-syllabus-index.md](../architecture/ai-syllabus-index.md) |
| 028-F | `028_f_ai_exam_pattern_learning.sql` | AI Exam Pattern Learning (SPEC 028-F): 5 tablas (`exam_pattern_analysis_runs`, `exam_pattern_profiles`, `topic_exam_patterns`, `ai_error_memories`, `ai_question_quality_scores`) con RLS de **solo gestión**; índice único de 1 perfil activo por oposición. Solo agregados/huellas anti-copia, nunca examen copiable. Ver [ai-exam-pattern-learning.md](../architecture/ai-exam-pattern-learning.md) |
| 031 | `031_material_storage.sql` | SPEC 029: bucket **privado** `materials` en Supabase Storage + política sobre `storage.objects` (solo gestores owner/admin). El original se abre con **URL firmada** corta emitida por la sesión del gestor; nunca público ni service-role. El control por oposición lo aplica el facade (`getMaterialFile`). |
| 033 | (no es migración SQL) | SPEC 033: **Edge Function** `generate-questions` (Deno) — generación de preguntas anclada a fuentes en SERVIDOR. **No añade migración**: reutiliza las columnas de trazabilidad de 028-E (`questions.material_section_id`/`source_reference_id`/`topic_source_reference_id`, `question_generation_runs.source_strategy`/`source_reference_ids`/`material_section_ids`) y el CHECK de estado de run de 023 (los estados de ciclo de vida de SPEC 033 se **mapean** a `completed`/`partial`/`failed`). La clave del proveedor y la service_role son secretos de servidor. Ver [server-side-question-generation.md](../architecture/server-side-question-generation.md) y [ai-provider-secrets.md](../security/ai-provider-secrets.md) |
| 030 | `032_ocr_vision_scanned_pdfs.sql` | SPEC 030: OCR & Vision para PDFs escaneados. Columnas aditivas de OCR en `materials` (`extraction_method`, `ocr_status`, `ocr_confidence`, `ocr_page_count`, `ocr_processed_pages`, `ocr_failed_pages`, `ocr_warning_count`) + ampliación del CHECK de `extraction_status` con `scanned_detected`/`ocr_processing`/`completed_ocr`/`completed_ocr_with_warnings`/`ocr_failed`; tablas internas `material_ocr_runs` + `material_ocr_pages` con RLS de **solo gestión** (el alumno no ve runs/páginas/confianza/errores/imágenes; scope por la oposición vía `opposition_workspace`/`can_manage_workspace`). Aditiva e idempotente. **Fase 1 = MockOcrProvider sin red**; el proveedor real entra por Edge Function en Fase 2. Ver [ocr-scanned-pdfs.md](../architecture/ocr-scanned-pdfs.md) |
